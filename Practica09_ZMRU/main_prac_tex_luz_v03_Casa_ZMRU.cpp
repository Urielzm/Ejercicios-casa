...
void LoadTextures() //carga de todas las texturas
{
	...
	frontal = generateTextures("Texturas/BibliotecaFrente1.jpg", 0);
	trasera = generateTextures("Texturas/BibliotecaTrasera.jpg", 0);
	superior= generateTextures("Texturas/rojo.jpg", 0);
	inferior = generateTextures("Texturas/rojo.jpg", 0);
	derecho = generateTextures("Texturas/BibliotecaDerecho1.jpg", 0);
	izquierdo = generateTextures("Texturas/BibliotecaIzquierdo1.jpg", 0);
...
void display(void) //En esta funcion jugamos con todas las luces, a cada tipo de luz se le creo un tipo de shader, pero no por eso se deben manejar los shaders así, se pueden mezclar. Para fines didacticos se hizo así
{
	...
	model = glm::scale(model, glm::vec3(4.0f, 2.0f, 2.0f));//escalamos
	...
	//lightingShader.setMat4("model", model);
	lightingShader.setInt("material_diffuse", izquierdo);
