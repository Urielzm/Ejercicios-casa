...
void animate(void)
{
	nucleo += 0.05f;//HACE QUE SE AUMENTE SOLO, INTRODUCCION A LA ANIMACIÓN.
	e1 += 0.8f;
	e2 += 0.7f;
...
void display(void)
{
	...
	//e1
	model = glm::rotate(model, glm::radians(e1), glm::vec3(1, 1, 1));//rotacion ver la funcion de animacion
	model = glm::translate(model, glm::vec3(2, 0, 0));//trasladando la matriz 
	...
	//e2
	model = glm::rotate(model, glm::radians(-e2), glm::vec3(1, 5, 2));//rotacion ver la funcion de animacion
	...
	model = glm::mat4(1.0f);//-> aqui es cuando 
	//e3
	model = glm::rotate(model, glm::radians(e1), glm::vec3(-2, 5, 2));//rotacion ver la funcion de animacion
	...
	model = glm::mat4(1.0f);//-> aqui es cuando
	//e5
	model = glm::rotate(model, glm::radians(-e2), glm::vec3(-2, 7, 2));//rotacion ver la funcion de animacion
	model = glm::translate(model, glm::vec3(3, 0, 0));
	model = glm::scale(model, glm::vec3(0.15f, 0.15f, 0.15f));
	projectionShader.setMat4("model", model);
	...
	model = glm::mat4(1.0f);//-> aqui es cuando
	//e6
	model = glm::rotate(model, glm::radians(-e1), glm::vec3(1, 5, 1));//rotacion ver la funcion de animacion
	model = glm::translate(model, glm::vec3(3, 0, 0));
	model = glm::scale(model, glm::vec3(0.15f, 0.15f, 0.15f));
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.4f, 0.2f, 0.3f));
	sphere_draw();//dibujamos mercurio
	model = glm::mat4(1.0f);//-> aqui es cuando
	//e7
	model = glm::rotate(model, glm::radians(e1), glm::vec3(1, 5, 1));//rotacion ver la funcion de animacion
	...
	model = glm::scale(model, glm::vec3(0.15f, 0.15f, 0.15f));
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.5f, 0.1f, 0.1f));
	sphere_draw();//dibujamos mercurio
	model = glm::mat4(1.0f);//-> aqui es cuando
